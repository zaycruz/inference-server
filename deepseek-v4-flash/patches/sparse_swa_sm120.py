...
class DeepseekSparseSWAMetadataBuilder(AttentionMetadataBuilder):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # FlashInfer 0.6.14's SM120 DSV4 decode kernel has no 256-wide
        # instantiation. DSpark needs 128 sliding-window positions plus its
        # draft block, which rounds to 256. Pad to the existing 512-wide
        # kernel; the metadata kernel fills the tail with -1 and swa_lens
        # preserves the true number of valid entries.
        self.noncausal_index_width = (
            512 if self.is_dspark and noncausal_index_width == 256
            else noncausal_index_width
        )
