    ...
    def _copy_cuda_inputs(self, request: PleOffloadRequest) -> None:
        ...
        with torch.accelerator.device_index(self.device.index):
            with torch.cuda.stream(self._d2h_stream):
                # sm120 workaround (vllm#53960): the cross-stream wait on
                # _input_ready_event deadlocks during kernel warmup — the event
                # never fires and the offload worker starves. Skip the wait and
                # use blocking copies so this staging thread is self-paced.
                with torch.cuda.nvtx.range("ple_offload.copy_input_ids"):
